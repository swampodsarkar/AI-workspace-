...
import { BarChart3, Upload, TrendingUp, TrendingDown, Download } from 'lucide-react'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2 text-sm">
                <span className="text-green-400 flex items-center gap-1"><TrendingUp size={14} /> +12.5%</span>
            ...
            <div className="h-48 flex items-end gap-3">
              {[40, 65, 45, 80, 55, 90, 70, 85, 60, 95, 75, 100].map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-md bg-gradient-to-t from-primary-600 to-primary-400 transition-all hover:opacity-80" style={{ height: `${h}%` }} />
                  <span className="text-xs text-dark-500">{['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][i]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="flex gap-2 mb-3">
              <input className="input" placeholder="Ask about data (e.g., 'What are the trends?')" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && analyze()} />
              <button className="btn-primary" onClick={analyze} disabled={loading}>{loading ? '...' : 'Analyze'}</button>
            </div>
            {result && (
        ...
        <div className="space-y-4">
          <div className="card">
            ...
            {[
              { label: 'Total Revenue', value: '$128.5K', change: '+12.5%', up: true },
              { label: 'Users', value: '24,893', change: '+8.2%', up: true },
              { label: 'Bounce Rate', value: '32.1%', change: '-3.4%', up: false },
              { label: 'Avg. Session', value: '4m 32s', change: '+5.7%', up: true },
            ].map((stat, i) => (
