...
export default function DataAnalysis() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center gap-2 mb-6"><Icons.BarChart3 className="text-primary-400" size={22} /><h1 className="text-xl font-semibold">Data Analysis</h1></div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="card">
            <div className="flex items-center justify-between mb-4"><h3 className="font-semibold">Revenue Overview</h3><span className="text-green-400 flex items-center gap-1 text-sm"><Icons.TrendingUp size={14} /> +12.5%</span></div>
          ...
          <div className="card">
            ...
            <div className="flex gap-2 mb-3">
              ...
              <button className="btn-primary flex items-center gap-2" onClick={analyze} disabled={loading}><Icons.Sparkles size={14} /> {loading ? '...' : 'Analyze'}</button>
            </div>
          ...
          <div className="card">
            ...
            {[{ label: 'Revenue', value: '$128.5K', change: '+12.5%', up: true }, { label: 'Users', value: '24,893', change: '+8.2%', up: true }, { label: 'Bounce Rate', value: '32.1%', change: '-3.4%', up: false }].map((s, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-dark-700 last:border-0">
                ...
                <span className={`text-xs flex items-center gap-1 ${s.up ? 'text-green-400' : 'text-red-400'}`}>{s.up ? <Icons.TrendingUp size={12} /> : <Icons.TrendingDown size={12} />}{s.change}</span>
              </div>
            ))}
          </div>
          <div className="card"><button className="btn-secondary w-full flex items-center justify-center gap-2 text-sm"><Icons.Upload size={14} /> Import Data</button></div>
        </div>
      </div>
    </div>
  )
